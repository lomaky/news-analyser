    ...
    try {
      if (analysis?.articles?.length > 12) {
        ...
        const model = genAI.getGenerativeModel({
          ...
          systemInstruction:
            'You are an assistant that creates the script in ENGLISH for a podcast between 2 people discussing the news from Colombia. first speaker is "Peter", second speaker is "Jane"',
        ...
        let geminiPrompt = `Create the script for a podcast in ENGLISH between 2 people discussing the following news from Colombia. Start the conversation with Jane saying 'Welcome to this podcast where we will chat about the news from Colombia' first speaker is 'Peter', second speaker is 'Jane'. Return the script in JSON.\n\n`;
        geminiPrompt += "News:\n\n";
        geminiPrompt += newsMd;
